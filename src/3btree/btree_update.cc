                ...
                const ups_key_t *key, BtreeStatistics::InsertHints &hints)
{
  ...
  if (IS_SET(hints.flags, UPS_HINT_APPEND) && hints.append_count > 5)
    pivot_at_end = true;
                ...
                const ups_key_t *key, BtreeStatistics::InsertHints &hints)
{
  ...
  int pivot = 0;
  if (IS_SET(hints.flags, UPS_HINT_APPEND) && old_node->is_leaf()) {
                ...
                bool force_prepend, bool force_append)
{
  ...
  switch (result.status) {
    case UPS_DUPLICATE_KEY:
      if (IS_SET(hints.flags, UPS_OVERWRITE)) {
        /* key already exists; only overwrite the data */
        if (!node->is_leaf())
          return UPS_SUCCESS;
      }
      else if (NOT_SET(hints.flags, UPS_DUPLICATE))
