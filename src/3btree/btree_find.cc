...
return_result:
     ...
     * is set: */
    if (key && is_approx_match && NOT_SET(flags, LocalCursor::kSyncDontLoadKey))
  ...
  int find(Context *context, Page *page, ups_key_t *key, uint32_t flags,
    ...
    *is_approx_match = 0;
    ...
    /* successfull match */
    if (cmp == 0 && (flags == 0 || IS_SET(flags, UPS_FIND_EQ_MATCH)))
      return slot;

    /* approx. matching: smaller key is required */
    if (IS_SET(flags, UPS_FIND_LT_MATCH)) {
      if (cmp == 0 && IS_SET(flags, UPS_FIND_GT_MATCH)) {
        *is_approx_match = BtreeKey::kLower;
        return slot + 1;
      }

      if (slot < 0 && IS_SET(flags, UPS_FIND_GT_MATCH)) {
        *is_approx_match = BtreeKey::kGreater;
        return 0;
      }
      *is_approx_match = BtreeKey::kLower;
