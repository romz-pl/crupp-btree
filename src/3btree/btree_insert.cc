...
struct BtreeInsertAction : public BtreeUpdateAction {
  ...
  ups_status_t run() {
    ...
    assert(IS_SET_ANY(hints.flags, UPS_DUPLICATE_INSERT_BEFORE
                                    | UPS_DUPLICATE_INSERT_AFTER
                                    | UPS_DUPLICATE_INSERT_FIRST
                                    | UPS_DUPLICATE_INSERT_LAST)
              ? IS_SET(hints.flags, UPS_DUPLICATE)
              : true);
    ...
    if (hints.leaf_page_addr
            && IS_SET_ANY(hints.flags, UPS_HINT_APPEND | UPS_HINT_PREPEND)) {
  ...
  ups_status_t append_or_prepend_key() {
     ...
     */
    if ((IS_SET(hints.flags, UPS_HINT_APPEND) && node->right_sibling())
            || (IS_SET(hints.flags, UPS_HINT_PREPEND) && node->left_sibling())
     ...
     */
    if (node->length() != 0) {
      if (IS_SET(hints.flags, UPS_HINT_APPEND)) {
      ...
      if (IS_SET(hints.flags, UPS_HINT_PREPEND)) {
